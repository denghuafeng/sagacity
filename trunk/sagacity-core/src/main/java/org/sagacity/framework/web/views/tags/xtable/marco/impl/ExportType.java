...
public class ExportType implements MarcoFacade {
	 ...
	 * 
	 * @see
	 * com.abchina.framework.web.views.tags.xtable.marco.TableMarcoExt#process
	...
	public String process(PageContext pageContext, XTableModel tableModel,
		...
		while (iter.hasNext()) {
			...
			exportFile=exportFile.concat(".").concat(exportModel.getExtName());
			exportAction = tableModel.getExportAction() + "&"
					+ tableModel.getExportParam() + "="
					+ URLEncoder.encode(exportModel.getParam());
			exportAction=exportAction.concat("&" + XTableConstants.IS_EXPORT_PARAM + "="
					+ URLEncoder.encode("true"));
			exportAction=exportAction.concat("&" + XTableConstants.XTABLE_EXPORT_FILE + "="
					+ exportFile);
