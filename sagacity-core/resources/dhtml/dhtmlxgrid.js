...
dhtmlXGridObject.prototype={
	...
	*/
	getAllRowIds:function(separator){
		...
		for (var i = 0; i < this.rowsBuffer.length; i++)ar.push(this.rowsBuffer[i].idd);
	...
	_refreshFromXML:function(xml){
		...
		for (var i = 0; i < rows.length; i++){
			...
			if (this._dload||this.rowsBuffer[0].length){
				...
				this._fillRowFromXML(this.rowsAr[id], row, tree, pid); else if (this._refresh_mode[1]){
	...
	_process_xml_row:function(r, xml){
		...
		if (this._ud_enabled){
			...
			for (var i = udCol.length-1;
				i >= 0;
				i--)this.setUserData(udCol[i].getAttribute("name"), udCol[i].firstChild
